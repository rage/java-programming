...
In object-oriented programming, the structure of a program is formed by the data it processes. On the other hand, in procedural programming, the structure of the program is formed by the functionality of the program: the program acts as a step-by-step guide for the described functionality to be performed. The program is executed one step at a time, and subroutines (methods) are called whenever necessary.
