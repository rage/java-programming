...
The menu object has an List as an instance variable to store the names of the dishes on the menu. The menu should provide the following methods:
...
Implement the `public void clearMenu()` method, which clears the menu. The `ArrayList` class has a method which is useful here. IntelliJ can hint at the available methods when you type the object name an a dot. Try to write `meals.` inside the method frame and see what happens.
...
- `public List<Person> getPersons()` - returns a list of the persons in the room.
